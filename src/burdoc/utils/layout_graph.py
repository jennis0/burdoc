...
class LayoutGraph(object):
    ...
    def __get_next_overlaps_from_projection(self, node: Node, matrix_slice: np.ndarray, transpose: bool = False):
        ...
        if not transpose:
            ...
            def overlap_func(e1, e2): return e1.element.bbox.x_overlap(
                e2.element.bbox, 'min')

            def reject_overlap_func(
                e1): return node.element.bbox.y_overlap(e1.element.bbox)

            def distance_func(e1, e2): return max(
                e2.element.bbox.y0 - e1.element.bbox.y1, 0)
        else:
            if node.element.bbox.x1 >= self.pagebound.x1 - 1:
                return []

            def overlap_func(e1, e2): return e1.element.bbox.y_overlap(
                e2.element.bbox, 'min')

            def reject_overlap_func(
                e1): return node.element.bbox.x_overlap(e1.element.bbox)
            def distance_func(e1, e2): return max(
                e2.element.bbox.x0 - e1.element.bbox.x1, 0.)
    ...
    def __build_graph(self):
        ...
        for e in self.nodes[1:]:
            ...
            e.right = self.__get_next_overlaps_from_projection(
                e, matrix_slice, True)
    ...
    def __str__(self):
        ...
        for n in self.nodes:
            ...
            text += "\nU: " + \
                ',\n   '.join(
                    ['(' + str(self.nodes[n2[0]]) + ',' + str(round(n2[1], 1)) + ')' for n2 in n.up])
            text += "\nL: " + \
                ',\n   '.join(['(' + str(self.nodes[n2[0]]) + ',' +
                              str(round(n2[1], 1)) + ')' for n2 in n.left])
            text += "\nR: " + \
                ',\n   '.join(['(' + str(self.nodes[n2[0]]) + ',' +
                              str(round(n2[1], 1)) + ')' for n2 in n.right])
            text += "\nD: " + \
                ',\n   '.join(['(' + str(self.nodes[n2[0]]) + ',' +
                              str(round(n2[1], 1)) + ')' for n2 in n.down])
            text += "\n"
