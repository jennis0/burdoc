...
class RulesTableStrategy(TableExtractorStrategy):
    ...
    def __init__(self, logger: logging.Logger):
        super().__init__('rules', logger)

    def requirements(self) -> List[str]:
        return ['page_numbers', 'pages','blocks']

    def _generate_table_candidates(self, page: Page, blocks: List[TextBlock]) -> List[List[TextBlock]]:
    ...
    def _create_table_from_candidate(self, page: Page, candidate: List[List[TextBlock]]) -> Optional[Table]:
    ...
    def extract_tables(self, page_numbers: List[int], pages: Dict[int, Page], blocks: Dict[int, List[TextBlock]]) -> Dict[int, Any]:
        tables = {}
        for pn in page_numbers:
            page = pages[pn]
            page_blocks = blocks[pn]
            tables[pn] = []

            candidates = self._generate_table_candidates(page, page_blocks)
            candidates.sort(key=lambda c: c[0][0].bbox.y0*10 + c[0][0].bbox.x0)

            for cand in candidates:
                skip = False
                for tab in tables[pn]:
