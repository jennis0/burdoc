...
from ..elements.content_objects import CSpan, Paragraph, CTitle, CImage, CTable, Aside, CHeaderFooter
...
class ContentParser(object):
    ...
    def parse_block(self, block: TextBlock, last_element: LayoutElement, last_parsed: Any) -> List[Any]:
        current_class = None

        line_start = 0
        line_end = len(block.items)
        ...
        block.items = block.items[line_start:line_end]

        if last_element and isinstance(last_parsed, Paragraph) and abs(block.bbox.y0 - last_element.bbox.y1) < 1:
            paras = [last_parsed]
            strip_first = True
            if isinstance(last_element, TextBlock):
                current_class = self.get_text_class(last_element.items[0].spans[0])
        else:
            paras = []
            strip_first = False
        ...
        for line in block.items:
            ...
            if cl == current_class:
                if isinstance(paras[-1], CTitle):
                    ...
                    if len(paras) == 0:
                        paras.append(Paragraph([line.spans]))
                    else:
    ...
    def parse_table(self, table: Table, last_element: LayoutElement, last_parsed: Any) -> List[CTable]:
        ctable = CTable([], [], None, None)
        if table.headers:
            for h in table.headers:
                ctable.headers.append(Paragraph([l.spans for l in h]))

            for row in table.values:
                ctable.values.append([])
                for column in row:
                    ctable.values[-1].append(Paragraph([l.spans for l in column]))
